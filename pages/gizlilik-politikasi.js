...
export default function PrivacyPolicy() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`${apiUrl}/api/legal/privacy`);
  ...
  return (
    <>
      <Head>
        <title>Gizlilik Politikası | Nomanoğlu Kuyumculuk</title>
        <meta name="description" content="Nomanoğlu Kuyumculuk gizlilik politikası ve kişisel veri koruma açıklamaları." />
      </Head>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              ...
              <div className="px-6 py-8">
                ...
                `}</style>
                <div dangerouslySetInnerHTML={{ __html: sanitize(pageData.content) }} />
              </div>
            </div>
          ) : (
